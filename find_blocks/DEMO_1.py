...
# 获取网站的HTML内容
html_development_page = driver.page_source
tree = etree.HTML(html_development_page)

# 使用 BeautifulSoup 解析页面源码
soup_block_page = BeautifulSoup(html_development_page, 'html.parser')
tables = soup_block_page.find_all('table', {'class': 'generalTbl'})  # 找到所有类名为generalTbl的表格

rows1 = tables[0].find_all('tr')  # 找到所有的行
row_count1 = len(rows1)
col_count1 = max(len(row3.find_all(['th', 'td'])) for row3 in rows1)  # 找到列数
print(f"表格行数：{row_count1}, 列数：{col_count1}")

for index2 in range(2, 5):
    path_to_development = '//*[@id="multiSelect"]/tr['+ str(index2) +']/td[1]'
    development_element = driver.find_element('xpath', path_to_development)
    # //*[@id="multiSelect"]/tr[7]/td[1]
    # //*[@id="multiSelect2"]/tr[33]/td[1]
    # 模拟鼠标移动到元素上并点击
    actions = ActionChains(driver)
    actions.move_to_element(development_element).click().perform()
                    ...
                    if index1 == row_count:
                        if index2 == col_count:
                            ...
                            element5 = driver.find_element('xpath',
                                                           '//*[@id="page-content"]/tbody/tr[1]/td[2]/table/tbody/tr/td/div/form/table[3]/tbody/tr[1]/td/a[' + str(
                                                               key) + ']')
